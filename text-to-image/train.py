...
def get_training_batch(batch_no, batch_size, image_size, z_dim,
	...
	if data_set == 'MS-COCO':
		...
		real_index = np.arange(real_images.shape[0])
		wrong_index = copy.deepcopy(real_index)
		while True:
			np.random.shuffle(wrong_index)
			if np.sum(wrong_index==real_index) < 1:
				break
		wrong_images = real_image[wrong_index]
		"""
